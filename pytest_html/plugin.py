...
class HTMLReport(object):
    ...
    def _generate_report(self, session):
        ...
        summary = [html.p(
            '运行 {0} 个测试用例在 {1:.2f} 分钟内. '.format(
                numtests, suite_time_delta / 60)),
            html.p('点击选中框以筛选结果',
                   class_='filter',
                   hidden='true')]
        ...
        cells = [
            html.th('结果',
                    ...
                    col='result'),
            html.th('用例位置', class_='sortable', col='name'),
            html.th('持续时间', class_='sortable numeric', col='duration'),
            html.th('链接')
        ]
        ...
        body = html.body(
            ...
            html.p('报告生成于 {0} {1}'.format(
                generated.strftime('%Y{y}%m{m}%d{d}').format(y='年',m='月',d='日'),
                generated.strftime('%H:%M:%S ')),
                html.a('pytest-html', href=__pypi_url__),
                ' v{0}'.format(__version__)),
            onLoad='init()')

        body.extend(self._generate_environment(session.config))  # 注释此行可以关闭Environment

        summary_prefix, summary_postfix = [], []
        session.config.hook.pytest_html_results_summary(
            prefix=summary_prefix, summary=summary, postfix=summary_postfix)
        body.extend([html.h2('测试概要')] + summary_prefix
    ...
    def pytest_terminal_summary(self, terminalreporter):
        terminalreporter.write_sep('-', '生成的HTML文件: {0}'.format(
            self.logfile))
